    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            order = Order.objects.filter(
                user=user, status=StatusOrder.objects.get(id=1)
            ).first()
            if not order:
                order = Order.objects.create(
                    user=request.user, status=StatusOrder.objects.get(id=1)
                ).save()
...
class CatalogView(ListModelMixin, GenericAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = (
            queryset.filter(price__range=(int(minPrice), int(maxPrice)))
            .order_by(sort)
            .all()
        )
...
class OrdersListCreateView(GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        orders = (
            Order.objects.select_related("status_order")
        ...
        for order in orders:
            ...
            for order_product in order_products:
                ...
                serialised = ProductShortSerializer(
                    product,
                    data={"count": order_product.count, "price": order_product.price},
                    partial=True,
                )
                if serialised.is_valid():
                    data_products.append(serialised.data)
            serialised = OrderSerializer(
                order, data={"products": data_products}, partial=True
            )
            if serialised.is_valid():
...
class OrderUpdateView(GenericAPIView):
    ...
    def get(self, request, id):
        ...
        for order_product in OrderProduct.objects.filter(order=order):
            product = Product.objects.get(id=order_product.product_id)
            data = {"count": order_product.count, "price": order_product.price}
            serialised = ProductShortSerializer(product, data=data, partial=True)
            if serialised.is_valid():
